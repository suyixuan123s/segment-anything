...
mask_path = r'E:\ABB\AI\yolov9\segment\output\blood_tube_object_0.49.png'  # 掩码图像
...
def extract_point_cloud(mask, depth_image, fx, fy, cx, cy):
    ...
    for v in range(h):
        for u in range(w):
            if mask[v, u] > 0:  # 掩码区域
                z = depth_image[v, u] / 1000.0  # 假设深度图以毫米为单位，转换为米
                if z == 0:  # 忽略无效深度
                    continue
                x = (u - cx) * z / fx
                y = (v - cy) * z / fy
...
if len(points) > 0:
    ...
    save_path = r'E:\ABB\segment-anything\point_cloud_data\blood_tube_point_cloud.ply'
